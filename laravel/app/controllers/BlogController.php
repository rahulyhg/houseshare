...
public function admin_categories() {
		...
		//die;
			...
			if(! empty($_POST)){
				...
				if($validator->fails() ) {
					Session::flash('errormessage', 'Category could not be updated, Please correct errors');
...
public function admin_category_add(){
	...
	/*	public function admin_index() {
		...
		*/
		public function admin_index_blog() {
			...
			$filters = array();
			$data =  Blog::sortable()->leftjoin('subjects', 'subjects.id', '=', 'blogs.subject_id');
			$search_sess = array();
			...
			$data =$data->select('blogs.*','subjects.name')->orderBy('created_at','DESC')->paginate(Config::get('constants.PAGINATION'));
	...
	public function admin_blog_comment($blog_id=null){
...
public function admin_blog_comment_edit($id = null,$blog_id = null){
		
		//die;
			$data = BlogComment::find($id);
			
			if(! empty($_POST)){
				$validator = BlogComment::validate(Input::all());
				if($validator->fails() ) {
					Session::flash('errormessage', 'Blog Comment could not be updated, Please correct errors');
					return Redirect::to('admin/blogs/comment/edit/'.$id.'/'.$blog_id)->withErrors($validator)->withInput(Input::all());
				} else {  
					$post_data = Input::all(); 
					$save_data  = $data->update($post_data); 
					if($save_data){ 
						return Redirect::to('admin/blogs/comment/'.$blog_id)->with('message', 'Blog Comment has been updated successfully!');
					}else{
						return Redirect::to('admin/blogs/comment/edit/'.$id.'/'.$blog_id)->with('errormessage', 'Blog Comment could not be updated, please try again!!');
					}
				}
			}
			
			return View::make('backend.blogs.admin_blog_comment_edit')->with(compact('data'));
		}	
