...
class UserController extends BaseController {  
	...
	public function who_interested(){
		...
		$my_ad_data =  $my_ad_data->select('properties.*','ad_interesteds.user_id as interesteds_user_id')->where('properties.user_id',Auth::id())->groupBy('properties.id')->orderBy('properties.created_at','desc')->paginate(Config::get('constants.PAGINATION'));
	...
	public function ad_images($id =null) { 
		...
		if(!empty($_POST)) {
			...
			$video_file = isset($_FILES['video'])?$_FILES['video']:'';
			$video_file_type = Input::get('video_file');
			
			//echo '<pre>'; print_r($video_file); die; 
			
			if(!empty($video_file) && $video_file_type) {
			
				$file_name 		= isset($_FILES['video']['name'])?$_FILES['video']['name']:'';
				$file_tmp_name 	= isset($_FILES['video']['tmp_name'])?$_FILES['video']['tmp_name']:'';
				$ext = pathinfo($file_name, PATHINFO_EXTENSION);
				$k=1;
				$ad_image_exists = DB::table('ad_images')->where('property_id',$id)->where('is_default',1)->count(); 
				if($video_file) {
				 
					$allowed_extensions = array("webm", "mp4");
					if(!in_array($ext,$allowed_extensions)) {
						return Redirect::to('ad-images/'.$id)->with('errormessage', 'Invalid video format., please try again!!');
					}
					
					$post_data = array();
					
					$filename = strtotime(date('Y-m-d H:i:s')).'_'.rand(111111111,999999999).'.'.$ext;
					$dir_path = 'upload/ads/'.$filename;
					if(move_uploaded_file($file_tmp_name, $dir_path)) {
						//$photo_name =  $user->videoUpload($image_val,'ads');
						$post_data['property_id'] = $id;
						$post_data['img_type'] = 2;
						...
						return Redirect::to('ad-images/'.$id)->with('errormessage', 'Video could not be upload successfully!');
					}
					
				} else {
					return Redirect::to('ad-images/'.$id)->with('errormessage', 'Please select video, please try again!!');
				}
