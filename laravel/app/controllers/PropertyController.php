<?php
    class PropertyController extends BaseController {  
		...
		public function __construct(){
			...
			},['except' => ['listing', 'search', 'place_an_ad','advertise_add','advertise_back','login_register','advertise-action','advertise_action','details','email_advertiser','advertise_success']]);
		}
		
		
		public function search($slug=null){  
			
			return View::make('frontend.properties.search',compact(''));
		} 
 		
		
		public function listing($slug=null){ 
		
			$get_ad_type = Config::get('constants.ADVERTISE_TYPE_NUMBER.'.$slug);
			...
			if(!empty($_POST)){
				...
				$search_sess['type_search'] = $type_search;
				...
				if(isset($_POST['post_code']) and $_POST['post_code']!=''){
					$post_code = $_POST['post_code'];
					$data = $data->where('properties.post_code', 'like','%'.trim( $post_code).'%');
					$search_sess['post_code'] = $post_code;
				}
				
				if(isset($_POST['area']) and $_POST['area']!=''){
					$area = $_POST['area'];
					$data = $data->where('properties.area', 'like','%'.trim( $area).'%');
					$search_sess['area'] = $area;
				}
				...
				if(Input::get('minimum_stay_type')!=''){
					$minimum_stay_type = Input::get('minimum_stay_type');
					$data = $data->where('properties.minimum_stay_type','>=',$minimum_stay_type);
					$search_sess['minimum_stay_type'] = $minimum_stay_type;
				}
				
				if(Input::get('maximum_stay_type')!=''){
					$maximum_stay_type = Input::get('maximum_stay_type');
					$data = $data->where('properties.maximum_stay_type','<=',$maximum_stay_type);
					$search_sess['maximum_stay_type'] = $maximum_stay_type;
				} 
				
				if(Input::get('days_available_type')!=''){
					$days_available_type = Input::get('days_available_type');
					$data = $data->where('properties.days_available_type',$days_available_type);
					$search_sess['days_available_type'] = $days_available_type;
				}
				
				if(Input::get('minimum_age')!=''){
					$minimum_age = Input::get('minimum_age');
					$data = $data->where('properties.minimum_age','>=',$minimum_age);
					$search_sess['minimum_age'] = $minimum_age;
				}
				
				if(Input::get('maximum_age')!=''){
					$maximum_age = Input::get('maximum_age');
					$data = $data->where('properties.maximum_age','<=',$maximum_age);
					$search_sess['maximum_age'] = $maximum_age;
				} 
				
				if(Input::get('is_gender')!=''){
					$is_gender = Input::get('is_gender');
					$data = $data->where('properties.is_gender',$is_gender);
					$search_sess['is_gender'] = $is_gender;
				} 
				
				if(Input::get('is_smoking')!=''){
					$is_smoking = Input::get('is_smoking');
					$data = $data->where('properties.is_smoking',$is_smoking);
					$search_sess['is_smoking'] = $is_smoking;
				} 
				
				if(Input::get('is_occupation')!=''){
					$is_occupation = Input::get('is_occupation');
					$data = $data->where('properties.is_occupation',$is_occupation);
					$search_sess['is_occupation'] = $is_occupation;
				} 
				
				if(Input::get('i_am_we_are')!=''){
					$i_am_we_are = Input::get('i_am_we_are');
					$data = $data->where('properties.i_am_we_are',$i_am_we_are);
					$search_sess['i_am_we_are'] = $i_am_we_are;
				} 
				
				if(Input::get('looking_for')!=''){
					$looking_for = Input::get('looking_for');
					$data = $data->where('properties.looking_for',$looking_for);
					$search_sess['looking_for'] = $looking_for;
				} 
			...
			if(Session::has('adssearch')=='') {
				if($get_ad_type) {
					$data = $data->where('properties.ad_type',$get_ad_type);
					$get_add_type = $get_ad_type;
				} else {
		...
		public function details($id=null){ 
			...
			$data = $data->select('properties.*','ad_interesteds.is_interested','languages.name as language_name','countries.name as country_name','users.first_name as user_first_name','users.twitter','users.facebook','users.linkedin','users.google_plus','users.last_name as user_last_name','users.email as user_email')->first();
		...
		public function admin_edit($id = null){
			...
			if(! empty($_POST)){
				...
				if($data->ad_type==3) {
					$validator = Property::validate_last(Input::all());
				} else {
					$validator = Property::validate(Input::all());
				}
				if($validator->fails() ) {
			...
			$languages = DB::table('languages')->orderBy('name','ASC')->lists('name','id');
			$nationality = DB::table('countries')->orderBy('name','ASC')->lists('name','id');
			return View::make('backend.properties.admin_edit')->with(compact('data','amenities','languages','nationality'));
		}	
