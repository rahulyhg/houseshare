...
class GeneralHelper {
	...
	public static function showVideo($file=null, $class='', $width='', $height='', $title='', $type='', $max_height = "360px",$divid= null){
	
		$title  = strtolower($title);
		$path	= 'upload/'. $type.$file; 
		if($file != '' and file_exists($path)){ 
			return '<a target="_blank" href="'.Config::get('constants.SITE_URL').$path.'">'.Config::get('constants.SITE_URL').$path.'</a>';		
		}else{
			return '--'; 
		}
	}  
