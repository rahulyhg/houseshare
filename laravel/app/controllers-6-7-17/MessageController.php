...
//---------Backend Message----------
 ...
 public function admin_index_message() {
				
			$filters = array();
			$data =  messages::sortable();
			$search_sess = array();
			...
			if(! empty($_POST)){
				if(isset($_POST['messages_title']) and $_POST['messages_title']!=''){
					$title = $_POST['messages_title'];
					$data = $data->where('messages.messages_title', 'like','%'.trim( $title).'%');
					$search_sess['messages_title'] = $title;
				}
